...
pub fn get_plugin() -> CustomInstaller {
    let metadataplugin: PluginMetaData = PluginMetaData {
        ...
        subtitle: "Enable/Disable Dnf5 Fastestmirror Option",
    ...
    CustomInstaller::create(
        ...
        &["dnf --dump-main-config | grep fastestmirror |cut -f2- -d '=' | grep 1"],
        ...
        &["pkexec dnf config-manager setopt fastestmirror=1"],
        
        // [3] Remove Commands:
        // Commands to run when the "Remove" button is clicked.
        &["pkexec dnf config-manager setopt fastestmirror=0"],
